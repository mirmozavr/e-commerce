...
class Product(models.Model):
    ...
    def save(self, *args, **kwargs):
        ...
        if img.height < Product.MIN_RESOLUTION[0] or img.width < Product.MIN_RESOLUTION[1]:
            raise MinResolutionErrorException('Image resolution is smaller than required')
        if img.height > Product.MAX_RESOLUTION[0] or img.width > Product.MAX_RESOLUTION[1]:
            raise MaxResolutionErrorException('Image resolution is bigger than required')
        # image = self.image
        # img = Image.open(image)
...
class Notebook(Product):
    ...
    processor_freq = models.CharField(max_length=255, verbose_name='Частота')
...
class Smartphone(Product):
    ...
    sd_volume_max = models.CharField(max_length=255, null=True, blank=True, verbose_name='Объем встраиваемой памяти')
